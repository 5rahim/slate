 ...
 * @deprecated
 * @param {string} entry
 * @param {QueryHookCreatorReturn<T | null>} queryHook
 * @returns {[T, boolean, boolean]}
 */
export function useCachedEntry<T>(entry: string, queryHook: QueryHookCreatorReturn<T | null>) {
   ...
   const cache = legacy_useGlobalCache()
 ...
 * @deprecated
 * @param {string} entry
 * @param {LazyQueryHookCreatorReturn<T | null>} queryHook
 * @returns {[((options?: (QueryLazyOptions<any> | undefined)) => void), T, boolean, boolean]}
 */
 ...
 */
export const legacy_useGlobalCache = () => {
...
/**
 * @deprecated
 * Manage the cache we want to eliminate
 */
